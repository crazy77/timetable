...
function renderTimetable() {
  ...
  for (const course of courses) {
    for (const t of course.times) {
      ...
      const td = tbody.querySelector(`td[data-day="${t.day}"][data-hour="${startH}"]`);
      if (!td) continue;
      ...
      block.innerHTML = `
        ...
        ${heightSpan >= 0.8 ? `<span class="course-room">${t.start}</span>` : ""}
      `;
...
function mergeContinuousBlocks() {
  ...
  for (const course of courses) {
    ...
    for (const day of DAYS) {
      ...
      const times = dayTimes[day].sort((a, b) => timeToMinutes(a.start) - timeToMinutes(b.start));
      ...
      for (const group of groups) {
        ...
        for (const t of group) {
          ...
          const td = tbody.querySelector(`td[data-day="${day}"][data-hour="${h}"]`);
          if (td) {
            const blocks = td.querySelectorAll(".course-block");
            blocks.forEach(b => {
              if (b.querySelector(".course-name")?.textContent === course.name) {
                b.remove();
              }
            });
          }
        ...
        // 병합된 블록 생성
        const td = tbody.querySelector(`td[data-day="${day}"][data-hour="${firstH}"]`);
        if (!td) continue;
...
function renderTodayBanner() {
  ...
  // 시간순 정렬
  dayCourses.sort((a, b) => timeToMinutes(a.time.start) - timeToMinutes(b.time.start));
  ...
  container.innerHTML = grouped.map((item, i) => {
    ...
    const isNow = isSelectedToday && (currentTime >= item.startTime && currentTime <= item.endTime);
    ...
    return `
      <div class="today-class-item animate-in"
        ...
        <span class="today-class-time">${item.startTime} - ${item.endTime}</span>
        <span class="today-class-name" style="color: var(${colorVar})">${item.course.name}</span>
      </div>
    `;
  }).join("");
  ...
  container.querySelectorAll(".today-class-item").forEach(el => {
    el.addEventListener("click", () => {
      const course = courses.find(c => c.id === parseInt(el.dataset.courseId));
  ...
  for (const day of DAYS) {
    ...
    for (const course of courses) {
      ...
      if (dayTimes.length > 0) {
        // 같은 과목 시간 합치기
        const sorted = dayTimes.sort((a, b) => timeToMinutes(a.start) - timeToMinutes(b.start));
        dayCourses.push({
          course,
...
function openModal(course) {
  ...
  const timeText = Object.entries(dayTimes).map(([day, times]) => {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("todayFold").addEventListener("click", () => toggleTodayFold());
